...
const uint16_t PROGMEM combo_osm_lalt[] = {TD(TD_QUOT_RALT), TD(TD_COMM_DASH), TD(TD_DOT_EXLM), COMBO_END};
...
bool rgb_matrix_indicators_advanced_user(uint8_t led_min, uint8_t led_max) {
    ...
    if (osm_lock & MOD_MASK_ALT || osm & MOD_MASK_ALT)
        if (LED_QUOT_RALT >= led_min && LED_QUOT_RALT < led_max)
            set_osm_led(LED_QUOT_RALT, osm_lock & MOD_MASK_ALT, now);
